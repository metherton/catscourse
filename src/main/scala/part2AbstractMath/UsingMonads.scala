...
object UsingMonads {
  ...
  /*
     ...
     (for Try it will return a Failure, for Option it will return None, for Future it will return a failed Future
     for Either it will be Left
     - the issueRequest method returns a M containing the string: "request (payload) has been returned", if the payload is less than 20 chars
  ...
  object OptionHttpService extends HttpService[Option] {
    ...
    override def issueRequest(connection: Connection, payload: String): Option[String] =
      if (payload.length <= 20) Some(s"request $payload has been returned") else None
  }

  val responseOption = OptionHttpService.getConnection(config).flatMap(conn => OptionHttpService.issueRequest(conn, "option http service"))
  ...
  object AggressiveHttpService extends HttpService[ErrorOr] {
    override def getConnection(cfg: Map[String, String]): ErrorOr[Connection] = {
      if (!cfg.contains("host") || !cfg.contains("port")) {
        Left(new RuntimeException("Connection could not be established, invalid connection"))
      } else {
    ...
    override def issueRequest(connection: Connection, payload: String): ErrorOr[String] = {
      if (payload.length <= 20) Right(s"Request $payload has been returned") else Left(new RuntimeException("Payload is too large"))
    }
  }
  ...
  val errorOrResponse: ErrorOr[String] = for {
    ...
    resp <- AggressiveHttpService.issueRequest(conn, "a normal payload")
  } yield resp
