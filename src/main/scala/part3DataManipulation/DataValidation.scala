...
object DataValidation {
  ...
  def validateNumber(n: Int): Validated[List[String], Int] =
    Validated.cond(n % 2 == 0, n, List("Number must be even"))
      ...
      .combine(Validated.cond(n <= 100, n, List("number must <= 100>")))
  ...
  object FormValidation {
    ...
    def passwordCheck(password: String): FormValidation[String] =
      Validated.cond(password.length >= 10, password, List("Password must be at least 10 chars long"))
